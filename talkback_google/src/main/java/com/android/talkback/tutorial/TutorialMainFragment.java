...
public class TutorialMainFragment extends Fragment implements AccessibilityEventListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedState) {
        ...
        if (BuildCompat.isAtLeastN() && TalkBackServiceTest.getInstance() != null) {
            TalkBackServiceTest.getInstance().addEventListener(this);
        ...
        switch (event.getEventType()) {
            ...
            case AccessibilityEvent.TYPE_TOUCH_INTERACTION_END:
                if (mOffButton.didHoverEnter() && !mParentLayout.didHoverEnter() &&
                        !mOtherViewHovered) {
                    if (TalkBackServiceTest.getInstance() != null) {
                        TalkBackServiceTest.getInstance().disableTalkBack();
                    }
                }
                break;
