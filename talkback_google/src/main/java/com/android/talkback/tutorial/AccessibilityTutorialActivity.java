...
public class AccessibilityTutorialActivity extends Activity implements TutorialNavigationCallback,
    ...
    public void onStart() {
         ...
         * enabled).
         */
        final int serviceState = TalkBackServiceTest.getServiceState();
         ...
         */
        if (serviceState == TalkBackServiceTest.SERVICE_STATE_SUSPENDED) {
            showAlertDialogAndFinish(R.string.tutorial_service_suspended_title,
                    R.string.tutorial_service_suspended_message);
            return;
        } else if ((serviceState == TalkBackServiceTest.SERVICE_STATE_INACTIVE)) {
        ...
        if (preferences.getBoolean(TalkBackServiceTest.PREF_FIRST_TIME_USER, true)) {
            final SharedPreferences.Editor editor = preferences.edit();
            editor.putBoolean(TalkBackServiceTest.PREF_FIRST_TIME_USER, false);
            editor.apply();
            onFirstTimeLaunch();
        }
    ...
    private void onFirstTimeLaunch() {
        final TalkBackServiceTest service = TalkBackServiceTest.getInstance();
        if (service == null) {
            return;
        }
