...
public class ProcessorVolumeStream implements AccessibilityEventListener,
        TalkBackServiceTest.KeyEventListener, VolumeButtonPatternDetector.OnPatternMatchListener {
    ...
    private TalkBackServiceTest mService;
    ...
    private void handleBothVolumeKeysLongPressed() {
        if (TalkBackServiceTest.isServiceActive() && switchTalkBackActiveStateEnabled()) {
    ...
    private void handleSingleTap(int button) {
        if (TalkBackServiceTest.isServiceActive() && attemptNavigation(button)) {
            return;
        }
