...
public class KeyComboManager implements TalkBackServiceTest.KeyEventListener,
        TalkBackServiceTest.ServiceStateListener {
    ...
    static final String CONCATINATION_STR = " + ";
    ...
    private static final boolean IS_IN_ARC = TalkBackServiceTest.isInArc();

    private static final int DEFAULT_KEYMAP = IS_IN_ARC ? R.string.default_keymap_entry_value :
            R.string.classic_keymap_entry_value;
    ...
    /**
     * Returns converted key code. This method converts the following key events.
     * - Convert KEYCODE_HOME with meta to KEYCODE_ENTER.
     * - Convert KEYCODE_BACK with meta to KEYCODE_DEL.
    ...
    private TalkBackServiceTestStateHelper mServiceStateHelper;
    ...
    private KeyComboManager(Context context) {
        ...
        mServiceStateHelper = new TalkBackServiceTestStateHelperApp();
    ...
    private void initializeDefaultPreferenceValues() {
        ...
        preferences.edit().putString(mContext.getString(R.string.pref_select_keymap_key),
                mContext.getString(DEFAULT_KEYMAP)).apply();
    }

    /**
     * Sets KeyboardShortcutDialogPreference for key events. If it's set, it can listen and consume
     * key events before KeyComboManager does. Sets null to remove current one.
     */
    public void setKeyboardShortcutDialogPreferenceForKeyEvents(
            KeyboardShortcutDialogPreference keyboardShortcutDialogPreference) {
        mKeyboardShortcutDialogPreference = keyboardShortcutDialogPreference;
    }
    ...
    public String getKeymap() {
        SharedPreferences preferences = SharedPreferencesUtils.getSharedPreferences(mContext);
        return preferences.getString(mContext.getString(R.string.pref_select_keymap_key),
                mContext.getString(DEFAULT_KEYMAP));
    }
    ...
    /**
     * Sets TalkBackServiceTestStateHelper for testing.
     */
    void setTalkBackServiceTestStateHelperForTesting(
            TalkBackServiceTestStateHelper serviceStateHelper) {
        mServiceStateHelper = serviceStateHelper;
    }
    ...
    /**
     * Sets key combo model.
     * TODO: replace this method with setKeymap.
     */
    ...
    /**
     * Returns corresponding action id to key. If invalid value is passed as key, ACTION_UNKNOWN
     * will be returned.
     * @param key
     * @return
     */
    ...
    private void appendModifiers(int modifier, StringBuilder sb) {
        appendModifier(modifier, KeyEvent.META_ALT_ON,
                mContext.getString(R.string.keycombo_key_modifier_alt), sb);
        appendModifier(modifier, KeyEvent.META_SHIFT_ON,
                mContext.getString(R.string.keycombo_key_modifier_shift), sb);
        appendModifier(modifier, KeyEvent.META_CTRL_ON,
                mContext.getString(R.string.keycombo_key_modifier_ctrl), sb);
        appendModifier(modifier, KeyEvent.META_META_ON,
                mContext.getString(R.string.keycombo_key_modifier_meta), sb);
    }

    /**
     * Appends string representation of target modifier if modifier contains it.
     */
    private void appendModifier(int modifier, int targetModifier, String stringRepresentation,
            StringBuilder sb) {
    ...
    public boolean onKeyEvent(KeyEvent event) {
        if (mKeyboardShortcutDialogPreference != null) {
            if (mKeyboardShortcutDialogPreference.onKeyEventFromKeyComboManager(event)) {
                return true;
            }
        }

        if (!mHasPartialMatch && !mPerformedCombo &&
                (!mMatchKeyCombo || mListeners.isEmpty())) {
            return false;
        }
    ...
    private KeyEvent convertKeyEventInArc(KeyEvent event) {
        switch (event.getKeyCode()) {
            ...
            case KeyEvent.KEYCODE_BACK:
                ...
                return new KeyEvent(event.getDownTime(), event.getEventTime(), event.getAction(),
    ...
    private boolean onKeyDown(KeyEvent event) {
        ...
        boolean isServiceActive = mServiceStateHelper.isServiceActive();
        ...
        for (Map.Entry<String, Long> entry : mKeyComboModel.getKeyComboCodeMap().entrySet()) {
            ...
            if (match == EXACT_MATCH) {
                for (KeyComboListener listener : mListeners) {
                    if (listener.onComboPerformed(getActionIdFromKey(entry.getKey()))) {
        ...
        if (hasModifier && triggerModifier == KeyEvent.META_META_ON) {
            if (mPreviousKeyComboCode == mCurrentKeyComboCode &&
                ...
                (mCurrentKeyComboCode == KeyComboManager.getKeyComboCode(
    ...
    public void onServiceStateChanged(int newState) {
        // Unfortunately, key events are lost when the TalkBackServiceTest becomes active. If a key-down
    ...
    public interface TalkBackServiceTestStateHelper {
        public boolean isServiceActive();
    }

    private class TalkBackServiceTestStateHelperApp implements TalkBackServiceTestStateHelper {
        @Override
        public boolean isServiceActive() {
            return TalkBackServiceTest.isServiceActive();
        }
    }
}
